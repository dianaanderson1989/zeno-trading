    ...
    <div className="space-y-4 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-xs lg:text-sm mt-0.5">Portfolio overview</p>
        </div>
        {/* Desktop CTAs */}
      ...
      {/* Portfolio card */}
      <div className="relative overflow-hidden rounded-2xl p-5 lg:p-6 border border-neon-green/20"
        ...
        <p className="text-slate-400 text-xs font-semibold uppercase tracking-widest mb-1">Total Portfolio Value</p>
        {walletsLoading
          ? <div className="h-10 w-44 rounded-xl shimmer" />
          : <p className="text-3xl lg:text-5xl font-black text-white font-mono tracking-tight">
              {formatCurrency(totalValue)}
            </p>}
        {/* Mobile quick actions */}
        <div className="flex gap-3 mt-4 lg:hidden">
          <button onClick={() => navigate('/binary')}
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-bold text-dark-950"
            style={{ background: 'linear-gradient(135deg, #00ff88, #00cc6a)' }}>
            <Zap size={14} /> Binary
          </button>
          <button onClick={() => navigate('/trade')}
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-bold border border-white/[0.08] text-slate-200 bg-white/[0.04]">
            <TrendingUp size={14} /> Trade
          </button>
        </div>
        <div className="hidden lg:flex gap-4 mt-4">
          <button onClick={() => navigate('/wallet')} className="text-xs text-neon-green/70 hover:text-neon-green transition-colors font-medium">Wallet →</button>
          <button onClick={() => navigate('/history')} className="text-xs text-slate-500 hover:text-slate-300 transition-colors">History →</button>
        </div>
      </div>

      {/* Mobile: balances + recent activity stacked */}
      <div className="lg:hidden space-y-4">
        {/* Top balances — compact */}
        {activeWallets.length > 0 && (
          <div className="card p-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-bold text-white">Balances</h2>
              <button onClick={() => navigate('/wallet')} className="text-xs text-neon-green/60">All →</button>
            </div>
            <div className="space-y-2.5">
              {activeWallets.slice(0, 4).map(w => (
                <div key={w.id} className="flex items-center gap-2.5">
                  {w.assets?.icon_url
                    ? <img src={w.assets.icon_url} alt="" className="w-7 h-7 rounded-full flex-shrink-0" />
                    : <div className="w-7 h-7 rounded-full bg-dark-600 flex items-center justify-center text-xs flex-shrink-0">{w.assets?.symbol?.[0]}</div>}
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-semibold text-slate-200">{w.assets?.symbol}</span>
                      <span className="text-sm font-mono text-slate-200">{formatCurrency(w.usdValue)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-slate-500">{formatNumber(w.balance, 4)}</span>
                      <span className={`text-xs ${w.change24h >= 0 ? 'text-neon-green' : 'text-neon-red'}`}>
        ...
        {/* Market — simple list on mobile */}
        <div className="card p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-bold text-white">Market</h2>
            <button onClick={() => navigate('/trade')} className="text-xs text-neon-green/60">Trade →</button>
          </div>
          <div className="space-y-3">
            {marketData.map(feed => (
              <div key={feed.id} className="flex items-center gap-3" onClick={() => navigate(`/trade/${feed.assets?.symbol}`)}>
                {feed.assets?.icon_url
                  ? <img src={feed.assets.icon_url} alt="" className="w-8 h-8 rounded-full flex-shrink-0" />
                  : <div className="w-8 h-8 rounded-full bg-dark-600 flex items-center justify-center text-xs flex-shrink-0">{feed.assets?.symbol?.[0]}</div>}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-slate-200">{feed.assets?.symbol}</p>
                  <p className="text-xs text-slate-500">{feed.assets?.name}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-mono text-slate-200">{formatCurrency(feed.price)}</p>
                  <p className={`text-xs font-mono ${feed.change_24h >= 0 ? 'text-neon-green' : 'text-neon-red'}`}>
        ...
        {transactions.length > 0 && (
          <div className="card p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/history')} className="text-xs text-neon-green/60">All →</button>
            </div>
            <div className="space-y-2.5">
              {transactions.map(tx => (
                <div key={tx.id} className="flex items-center gap-2.5">
                  ...
                  <p className="text-xs font-mono text-slate-300 flex-shrink-0">
      ...
      <div className="hidden lg:grid lg:grid-cols-3 gap-6">
                    ...
                    <div key={w.id} className="flex items-center justify-between py-2.5 border-b border-white/[0.04] last:border-0">
                      <div className="flex items-center gap-2.5">
                        {w.assets?.icon_url
                          ...
                          : <div className="w-7 h-7 rounded-full bg-dark-600 flex items-center justify-center text-xs">{w.assets?.symbol?.[0]}</div>}
                        <div>
                      ...
                      <div className="text-right">
                        ...
                        <p className={`text-xs font-mono ${w.change24h >= 0 ? 'text-neon-green' : 'text-neon-red'}`}>{formatPercent(w.change24h)}</p>
                      </div>
                    </div>
                  ))}
                </div>}
        ...
        <div className="lg:col-span-2 card overflow-hidden p-0">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {Object.values(prices).filter(p => p.assets?.symbol !== 'USDT').map(feed => (
                <tr key={feed.id} className="table-row-hover border-b border-white/[0.03] last:border-0">
                  ...
                  <td className="py-3.5 px-5 text-right">
                    <span className={`inline-flex items-center gap-1 text-xs font-mono font-bold px-2 py-1 rounded-lg ${feed.change_24h >= 0 ? 'bg-neon-green/10 text-neon-green' : 'bg-neon-red/10 text-neon-red'}`}>
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <button onClick={() => navigate(`/trade/${feed.assets?.symbol}`)} className="text-[10px] text-neon-green/60 hover:text-neon-green font-semibold flex items-center gap-0.5 ml-auto">
