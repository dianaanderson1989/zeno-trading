      ...
      <div className="flex-1 flex items-center gap-5 overflow-hidden">
        {tickerFeeds.map(feed => (
          <div key={feed.asset_id} className="flex items-center gap-2 flex-shrink-0 text-xs">
            ...
            <span className={`font-mono font-semibold ${feed.change_24h >= 0 ? 'text-neon-green' : 'text-neon-red'}`}>{formatPercent(feed.change_24h)}</span>
          </div>
        ))}
      </div>
      <div className="flex items-center gap-1">
        <NotificationBell />
        <button onClick={signOut} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-500 hover:text-slate-200 hover:bg-white/[0.06] rounded-lg transition-colors">
