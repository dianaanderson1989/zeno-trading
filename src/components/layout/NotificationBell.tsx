    ...
    <div className="relative" ref={panelRef}>
      ...
      {open && (
        <div className="absolute right-0 top-10 w-80 z-50 animate-slide-up"
          style={{ background: 'rgba(10,15,30,0.98)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-white">Notifications</span>
              {unreadCount > 0 && <span className="badge bg-neon-green/10 text-neon-green border border-neon-green/20 text-xs">{unreadCount} new</span>}
            </div>
            <div className="flex items-center gap-1">
              {unreadCount > 0 && (
                <button onClick={markAllRead} className="flex items-center gap-1 text-xs text-slate-500 hover:text-neon-green px-2 py-1 rounded-lg transition-colors">
                  <CheckCheck size={12} /> All read
                </button>
              )}
              <button onClick={() => setOpen(false)} className="text-slate-600 hover:text-slate-300 p-1"><X size={13} /></button>
            </div>
          </div>
                ...
                {notifications.map((n: Notification) => {
                  ...
                  return (
                    <div key={n.id} onClick={() => { if (!n.is_read) markRead(n.id) }}
                      className={`flex gap-3 px-4 py-3.5 cursor-pointer hover:bg-white/[0.03] transition-colors ${!n.is_read ? 'bg-neon-green/[0.03]' : ''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {!n.is_read && <span className="w-1.5 h-1.5 rounded-full bg-neon-green flex-shrink-0 mt-1" />}
                        </div>
          ...
          {notifications.length > 0 && (
            <div className="px-4 py-2.5 border-t border-white/[0.06] text-center">
              <p className="text-[10px] text-slate-600">Refreshes every 30 seconds</p>
            </div>
          )}
        </div>
      )}
